...
export class EditarConsursoComponent implements OnInit {
  ...
  createFormGroup(){
    return new FormGroup({
      id:new FormControl(''),
      titulo:new FormControl('',[Validators.required,Validators.minLength(6),Validators.pattern(this.letras)]),
  ...
  constructor(
    public dialogRef: MatDialogRef<EditarConsursoComponent>,@ Inject(MAT_DIALOG_DATA) public data: formularioConcurso,private concurso:ConcursoService) {
      ...
      this.formulario.controls['gestion'].setValue(data.gestion);
    ...
    error_titulo() {
      ...
      return this.titulo.hasError('pattern') ? 'Solo se aceptan letras' : '';
    }
    error_gestion() {
      ...
      if(this.gestion.hasError('min'))
        return 'Ingrese minimo 3 caracteres';
      return this.gestion.hasError('pattern') ? 'Solo se aceptan letras' : '';
    }
    error_fecha() {
